...
class PrimerParcial2021 {
	...
	void queSePuedaCrearUnaPelicula() {
		...
		final String GENERO_ESPERADO = "Accion";
        ...
        Actores ricardo= new Actores("Ricardo Darin");
        Actores susana= new Actores("Susana Gimenez");
		pelicula.agregarActor(ricardo);
		pelicula.agregarActor(susana);
		...
		assertTrue(pelicula.actua(ricardo));
	...
	void queSePuedaCrearUnVideojuego() {
		...
		final String CONSOLA_ESPERADA = "Playstation";
		...
		assertEquals(CONSOLA_ESPERADA, juego.getConsola());
	...
	void queUnaPeliculaSeaVendible() {
		...
		// Ejecución
		Pelicula pelicula = new Pelicula(CODIGO_ESPERADO, DESCRIPCION_ESPERADA, GENERO_ESPERADO, ANO_DE_ESTRENO_ESPERADO, DIRECTOR_ESPERADO);
		pelicula.precioDeVenta(PRECIO_VENTA);
	...
	void queUnJuegoSeaAlquilable() {
		...
		// Ejecución
		Videojuego juego = new Videojuego(CODIGO_ESPERADO, DESCRIPCION_ESPERADA, CONSOLA_ESPERADA);
		juego.precioDeAlquiler(PRECIO_ALQUILER);
